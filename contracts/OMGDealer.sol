...
contract OMGDealer is Claimable, Whitelist, S3DEvents, S3DTokenBase{ 
    ...
    */
    function OMGDealer(address _tokenAddress)
        public
    {
        tokenPriceInitial = 0.000008 * 1e18;
        tokenPriceIncremental = 0.0000008 * 1e18;

        OMGTokenAddress = _tokenAddress;
     ...
     */
    function buy(address _buyer, uint256 _amountTokens, address _referredBy)
        public
        payable
        ...
        returns(uint256)
    {
        ...
        OMGTokenInterface(OMGTokenAddress).transferFrom(_buyer, address(this), _amountTokens);
        purchaseTokens(_buyer, _amountTokens, _referredBy);
    }
    ...
    */
        ...
        onlyIfWhitelisted(msg.sender)
    {
        ...
        OMGTokenInterface(OMGTokenAddress).transfer(_buyer, _withdrawAmount);
        ...
        onlyIfWhitelisted(msg.sender)
    {
        ...
        uint256 _omgAmount = s3dToBuyTokens(_tokens);
        uint256 _dividends = SafeMath.div(_omgAmount, dividendFee);
        uint256 _taxedOmg = SafeMath.sub(_omgAmount, _dividends);
        ...
        int256 _updatedPayouts = (int256) (profitPerShare * _tokens + (_taxedOmg * magnitude));
        ...
        // fire event
        emit S3DEvents.onTokenSell(_customerAddress, _tokens, _taxedOmg);
    }
    ...
    =            INTERNAL FUNCTIONS            =
    ==========================================*/
    function purchaseTokens(address _buyer, uint256 _incomingOmg, address _referredBy)
        internal
        antiEarlyWhale(_buyer, _incomingOmg)
        returns(uint256)
    {
        // data setup
        //address _customerAddress = buyer;
        uint256 _undividedDividends = SafeMath.div(_incomingOmg, dividendFee);
        uint256 _referralBonus = SafeMath.div(_undividedDividends, 3);
        uint256 _dividends = SafeMath.sub(_undividedDividends, _referralBonus);
        uint256 _taxedOmg = SafeMath.sub(_incomingOmg, _undividedDividends);
        uint256 _amountOfTokens = buyTokensToS3d(_taxedOmg);
        ...
        // fire event
        emit S3DEvents.onTokenPurchase(_buyer, _incomingOmg, _amountOfTokens, _referredBy);
        ...
        returns(uint256)
    {
        ...
        // russian hackers BTFO
        uint256 _omgAmount = s3dToBuyTokens(_amountOfTokens);
        require(_omgAmount <= totalBalance());        
        ...
        OMGTokenInterface(OMGTokenAddress).transfer(_customerAddress, _taxedOmg);

        // fire event
        emit S3DEvents.onTokenArbitrage(_customerAddress, _amountOfTokens, _taxedOmg);
        return _taxedOmg;
    }
}